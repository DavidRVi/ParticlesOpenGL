...
cParticleSystemTwo::cParticleSystemTwo() {}
cParticleSystemTwo::~cParticleSystemTwo(){}

bool cParticleSystemTwo::Load()
{
	int i;

	if (!text.Load("p10.png")) return false;
...
bool cParticleSystemTwo::MakeTexture()
{
	int width = 256,
		height = 256;
...
void cParticleSystemTwo::InitParticle(int i)
{
	float v, angle;
	...
	particle[i].dir = cVector3D(sin(angle) *v, cos(angle)* v, 0.0f);
}

void cParticleSystemTwo::Update()
{
	int loop;
...
void cParticleSystemTwo::Render()
{
	int loop;
	float x, y, z;
